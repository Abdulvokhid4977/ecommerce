    ...
    if (event.isSearchCompleted) {
      ...
      try {
        ...
        if (responses.every((response) =>
          ...
          emit(SearchSuccess(result1, category: result2));
        }
      } catch (s, e) {
        if (kDebugMode) {
          print('$s,$e');
    ...
    if (event.fromNavBar) {
      ...
      try {
        ...
        if (response.statusCode >= 200 && response.statusCode < 300) {
          ...
          emit(
            SearchError(
                'This is coming from FetchCategoryProductState: ${response.statusCode}'),
          );
        }
      } catch (e, s) {
        if (kDebugMode) {
        ...
        if (response.statusCode < 200 || response.statusCode >= 300) {
          ...
          emit(currentState.copyWith(
            product: currentState.product.copyWith(
                product: _updateProductsList(currentState.product.product,
                    event.productElement.id, !event.isFavorite)),
          ));
